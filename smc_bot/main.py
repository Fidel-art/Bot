    ...
    def run_analysis_cycle(self) -> None:
        ...
        try:
            ...
            for symbol in self.selected_symbols:
                ...
                # Persist drawings generated by analysis (so dashboard can render zones)
                try:
                    drawings = trade_signal.get('drawings', [])
                    if drawings:
                        db = _get_dashboard_db()
                        if db:
                            trader_id_for_db = self.trader_profile.trader_id if self.trader_profile else 'default'
                            for draw in drawings:
                                d = draw.copy()
                                d['symbol'] = symbol
                                db.save_drawing(trader_id_for_db, d)
                except Exception:
                    pass
                    ...
                    try:
                        ...
                        if db:
                            db.save_trade(self.trader_profile.trader_id if self.trader_profile else 'default', {
                                ...
                                # Persist drawings (if any) for dashboard visualization
                                try:
                                    drawings = trade_signal.get('drawings', [])
                                    trader_id_for_db = self.trader_profile.trader_id if self.trader_profile else 'default'
                                    for draw in drawings:
